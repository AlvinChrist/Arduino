...
void setup() {
  ...
  for (int i = 0; i < 100; i++) {
    ...
    if (jarak > 50 && jarak < 1000) {
      range_thres += jarak;
      c += 1;
    }
  }
  range_thres = (range_thres/c)+10;
}
...
void Sanitize(bool s = false) {
  ...
  while ((jarak > range_thres && jarak < 1000)|| s == true) {
    jarak = sensor.readRangeContinuousMillimeters();
    if ((jarak > range_thres && jarak < 1000) || s == true) {
      digitalWrite(BLUE_LED, HIGH);
    }
    else {
...
void loop() {
  ...
  if ((jarak > range_thres && jarak < 1000)) {
    Serial.println(jarak);
    Sanitize();
  }
  else {
