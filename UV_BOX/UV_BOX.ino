...
MCUFRIEND_kbv tft;
const char *logo = "/dm.bmp";
const char *_info = "Info";
const char *back = "<-";
const char *_mulai = "Mulai";
const char *_menit = "menit";
const char *left = "<";
const char *right = ">";
const char *dm = "---DOMOTECH---";
const char *judul = "UV BOX By DOMOTECH";
const char *call = "Call Us 081807928888";
const char *selesai = "Proses Selesai!";
const char *berlansung="sedang berlansung..";
const char *proses_mulai = "sedang berlansung..";
const char *ig ="IG @domotechbymeraki";
...
uint8_t showBMP(char *nm, int x, int y)
{
  ...
  uint8_t bmpDepth, sdbuffer[3 * BUFFPIXEL], bitmask;        // Bit depth (currently must be 24
  uint32_t bmpImageoffset, rowSize, pos;  // Start of image data in file
  uint16_t n, lcdbuffer[(1 << PALETTEDEPTH) + BUFFPIXEL], *palette = NULL;
...
bool Touch_getXY(void)
{
  ...
  if (pressed) {
    ...
    pixel_x = map(p.y, 85, 897, 0, 320); //LANDSCAPE kbv
...
void Home() {
  clr();
  showmsg(52, 10, 2, GREENYELLOW, judul);
  tft.drawFastHLine(0, 10 + 20, tft.width(), OLIVE);
  showBMP(logo, 120, 40);
  on_btn.initButton(&tft, 220, 170 , 40, 40, BLUE, BLUE, WHITE, right, 2);
  off_btn.initButton(&tft, 100, 170 , 40, 40, BLUE, BLUE, WHITE, left, 2);
  start_btn.initButton(&tft, 260, 220, 90, 40, OLIVE, OLIVE, NAVY, _mulai, 2);
  info_btn.initButton(&tft, 60, 220, 90, 40, DARKGREY, DARKGREY, BLUE, _info, 2);
  on_btn.drawButton();
  off_btn.drawButton();
  start_btn.drawButton();
  info_btn.drawButton();
  showmsg(130, 170, 2, GREEN, _menit);
...
void Info() {
  ...
  back_btn.initButton(&tft, 40, 218, 60, 40, BLACK, DARKGREY, BLACK, back, 3);
  back_btn.drawButton();
  //showmsg((tft.width() - 12 * 14) / 2, tft.height() * 2 / 5, 2, GREENYELLOW , "---DOMOTECH---");
  showmsg(33, 76, 3, GREENYELLOW , dm);
...
void loop(void)
{
  ...
  if (millis() - prevMillis >= 60000 && idle == false) {
    ...
    if (start_btn.justPressed() && ((PIND & (1 << PIND1)) >> PIND1) == 0) {
      ...
      showmsg(70, 110, 2, GREEN, selesai);
      delay(3000);
      Home();
    }
  }
