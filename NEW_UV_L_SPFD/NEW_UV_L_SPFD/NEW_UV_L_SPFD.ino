...
#define ORIENTATION 3 //0 PORTRAIT 1 LANDSCAPE
...
bool Touch_getXY(void)
{
  ...
  DDRC = DDRC | B00000110;  //for spfd
  PORTC = PORTC | B0000110;
  ...
  if (pressed) {
    ...
    //pixel_x = map(p.y, 51, 852, 0, 320); //spfd5408
    //pixel_y = map(p.x, 957, 13, 0, 240);
    pixel_x = map(p.y, 175, 909, 0, 320); //spfd5408
    pixel_y = map(p.x, 937, 193, 0, 240);
  }
  return pressed;
}
