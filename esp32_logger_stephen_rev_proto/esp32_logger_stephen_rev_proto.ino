...
//const char* ssid = "TP-LINK_FE4B7A-2.4G";
...
boolean paket_sensor=false, paket_kontrol=false;
char pisah[] = "";
...
void baca_kontrol() {
  if ((WiFi.status() == WL_CONNECTED)) { //Check the current connection status
    ...
    if (httpCode > 0) { //Check for the returning code
      ...
      Serial.print("Error on HTTP request : " + String(httpCode));
    }
...
void kirim_sensor(int a[]) {
  if ((WiFi.status() == WL_CONNECTED)) { //Check the current connection status
    HTTPClient http;
    String isi = "http://kotabaru.pertaminasafeteams.com/data_1/input.php?s1=";
    for (int i = 0; i < 10; i++) {
      isi += a[i];
      (i < 9) ? isi += "&s" + String(i + 2) + "=" : "";
    }
    /*
      isi += a[0];
      isi += "&s2=";
